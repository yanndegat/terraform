...
func (g *Graph) walk(walker GraphWalker) error {
	...
	debug := DebugInfo.Writer("walk-" + path)
	defer debug.Close()
	...
	walkFn = func(v dag.Vertex) (rerr error) {
		...
		debug.Printf("[DEBUG] vertex %T(%s.%s): walking\n", v, path, dag.VertexName(v))
		...
		if ev, ok := v.(GraphNodeEvalable); ok {
			...
			debug.Printf("[DEBUG] vertex %T(%s.%s): evaluating\n", v, path, dag.VertexName(v))
		...
		if ev, ok := v.(GraphNodeDynamicExpandable); ok {
			log.Printf(
				...
				path,
				dag.VertexName(v))
			debug.Printf("[DEBUG] vertex %T(%s.%s): expanding\n", v, path, dag.VertexName(v))
